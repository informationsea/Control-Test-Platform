...
void controller_simulation_step(void)
{
    ...
    double torque_master = target_distance*0.8 - target_distance_diff*400;
    if (torque_master > 1)
        torque_master = 1;
    // if (ABS(target_angle) > M_PI/4)
    //     torque_master = 0.5;
    double torque_diff = (target_angle/M_PI*0.1 - target_angle_diff*800)*(torque_master*0.5+0.5);
#define MAX_TORQUE_DIFF 1.0
